...
login()
http://27.221.75.201:8888/api/login?username=zzmdianjingyun2&password=123456
%slogin?username=%s&password=%s
0|登陆成功|token=bb7bdaaaec034ed1986f1c2620afb848


getphone()
http://27.221.75.201:8888/api/getjmphone?yysxz=0&token=bb7bdaaaec034ed1986f1c2620afb848&qylx=0&xmid=31&qygjz=&yys=0&qyxz=0&author=zzmdianjingyun2
%sgetjmphone?yysxz=0&token=%s&qylx=0&xmid=%s&qygjz=&yys=0&qyxz=0&author=zzmdianjingyun2
0|17131634484|浙江|宁波

getsms()
http://27.221.75.201:8888/api/jmgetMessage?token=bb7bdaaaec034ed1986f1c2620afb848&xmid=31&phone=17131634484
...
release()
http://27.221.75.201:8888/api/jmSpecified?token=bb7bdaaaec034ed1986f1c2620afb848&xmid=31&phone=17131634484
...
def getphone(ITEMID, PHONENUM=''):
    ...
    2指定号码（电竞云专用，因为他的指定接码调用的是不同的function...)
    http://27.221.75.201:8888/api/jmDesignated?token=bb7bdaaaec034ed1986f1c2620afb848&xmid=31&phone=17821984951&author=zzmdianjingyun2


    '''

    if PHONENUM == '':
        url = '%sgetjmphone?yysxz=0&token=%s&qylx=0&xmid=%s&qygjz=&yys=0&qyxz=0&author=zzmdianjingyun2' % (
        api_address, TOKEN, ITEMID)

    else:
        url = '%sjmDesignated?token=%s&xmid=%s&phone=%s&author=zzmdianjingyun2' % (api_address, TOKEN, ITEMID, PHONENUM)
    ...
    if key == "1":  # TOKEN错误，重新登录 (电竞云不需要，因为不是token规定)
